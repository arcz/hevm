...
currentContract vm =
  view (#env . #contracts . at vm.state.codeContract) vm
...
makeVm o =
  ...
  , state = FrameState
    ...
    , memory = mempty
    , memorySize = 0
    , code = o.vmoptContract.contractcode
    , contract = o.vmoptAddress
    , codeContract = o.vmoptAddress
    , calldata = fst o.vmoptCalldata
    , callvalue = o.vmoptValue
    , caller = o.vmoptCaller
    , gas = o.vmoptGas
    , returndata = mempty
    , static = False
    }
  , env = Env
    { sha3Crack = mempty
    , chainId = o.vmoptChainId
    , storage = if o.vmoptStorageBase == Concrete then EmptyStore else AbstractStore
    , origStorage = mempty
  ...
  , burned = 0
  , constraints = snd o.vmoptCalldata
  , keccakEqs = mempty
  , iterations = mempty
...
initialContract theContractCode = Contract
  { contractcode = theContractCode
  , codehash = hashcode theContractCode
  , balance  = 0
  , nonce    = if creation then 1 else 0
  , opIxMap  = mkOpIxMap theContractCode
  , codeOps  = mkCodeOps theContractCode
  , external = False
  } where
              ...
              accessStorage self x $ \current -> do
                ...
                if num availableGas <= g_callstipend
                  then finishFrame (FrameErrored (OutOfGas availableGas (num g_callstipend)))
                    ...
                    let original = case readStorage (litAddr self) x (ConcreteStore vm.env.origStorage) of
                                     Just (Lit v) -> v
                                     _ -> 0
...
precompiledContract this xGas precompileAddr recipient xValue inOffset inSize outOffset outSize xs =
  ...
  do
    ...
    self <- gets (.state.contract)
    stk <- gets (.state.stack)
    pc <- gets (.state.pc)
    case stk of
...
finalize = do
  let
    revertContracts  = gets (.tx.txReversion) >>= assign (#env . #contracts)
  ...
  gets (.result) >>= \case
    ...
    Just (VMSuccess output) -> do
      ...
      pc' <- gets (.state.pc)
      creation <- gets (.tx.isCreate)
      createe  <- gets (.state.contract)
      createeExists <- (Map.member createe) <$> gets (.env.contracts)
  ...
  txOrigin     <- gets (.tx.origin)
  sumRefunds   <- (sum . (snd <$>)) <$> (gets (.tx.substate.refunds))
  miner        <- gets (.block.coinbase)
  blockReward  <- num <$> (gets (.block.schedule.r_block))
  gasPrice     <- gets (.tx.gasprice)
  priorityFee  <- gets (.tx.txPriorityFee)
  gasLimit     <- gets (.tx.txgaslimit)
  gasRemaining <- gets (.state.gas)
...
accessAccountForGas addr = do
  accessedAddrs <- gets (.tx.substate.accessedAddresses)
...
accessStorageForGas addr key = do
  accessedStrkeys <- gets (.tx.substate.accessedStorageKeys)
...
cheat (inOffset, inSize) (outOffset, outSize) = do
  vm <- get
  let
    abi = readBytes 4 (Lit inOffset) vm.state.memory
          ...
          fetchAccount xTo' $ \target ->
                burn xGas $ do
                  let newContext = CallContext
                                    ...
                                    , callContextCodehash  = target.codehash
                                    , callContextReversion = (vm0.env.contracts, vm0.env.storage)
                                    , callContextSubState  = vm0.tx.substate
                                    , callContextAbi =
                                        if xInSize >= 4
        ...
        let
          ...
          newContext  =
            CreationContext { creationContextAddress   = newAddr
                            , creationContextCodehash  = newContract.codehash
                            , creationContextReversion = vm0.env.contracts
                            , creationContextSubstate  = vm0.tx.substate
                            }
    ...
    nextFrame : remainingFrames -> do
      ...
      case nextFrame.context of
        ...
        CallContext _ _ (Lit -> outOffset) (Lit -> outSize) _ _ _ reversion substate' -> do
          ...
          touched <- gets (.tx.substate.touchedAccounts)
