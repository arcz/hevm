...
initVm bs = vm
  where
    ...
    vm = makeVm $ VMOpts
      { vmoptContract      = c
      , vmoptCalldata      = (AbstractBuf "txdata", [])
      , vmoptValue         = CallValue 0
      , vmoptAddress       = Addr 0xffffffffffffffff
      , vmoptCaller        = Lit 0
      , vmoptOrigin        = Addr 0xffffffffffffffff
